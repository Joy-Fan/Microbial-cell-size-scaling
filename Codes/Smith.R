...
Data <- read.csv("Smith.csv", as.is=TRUE)
Data<-Data[-which(Data$Superkingdom=="Phytoplankton"),]
...
png(filename = "V.jpg",width =1000, height =1000)
...
#Figure
png(filename = "Hira_Breakpoint.jpg",width =1200, height =1000) 
...
summary(fit)
Coef_Hira_MesoB<-tidy(summary(fit))
Meso1$a<--0.41720 
Meso1$E<-0.57397
Coef_Hira_MesoB$R2<- 0.5227
Coef_Hira_MesoB$P<-3.947e-12
Coef_Hira_MesoB$Superkingdom<-"Bacteria"
...
summary(fit)
Coef_Hira_MesoA<-tidy(summary(fit))
Meso2$E<-  0.003928
Meso2$a<-0.315226
Coef_Hira_MesoA$R2<-  0.3477 
Coef_Hira_MesoA$P<- 0.01126
Coef_Hira_MesoA$Superkingdom<-"Archaea"

#Merge the data Data_T2
Coef_Hira_M_Breakpoint<-rbind(Coef_Hira_MesoA,Coef_Hira_MesoB)
write.table(Coef_Hira_M_Breakpoint,"C:/Users/FAN/Desktop/Final/Tables/Coef_Hira_M_Breakpoint.csv",sep=",")

#Figure 2b
setwd("C:/Users/FAN/Desktop/Final/Figures")
Meso<-rbind(Meso1,Meso2)
Meso<-Meso %>% mutate(TCorrected_GrowthRate = GrowthRate/(exp(1)^(-E/(k*OptimumTemp))))
png(filename = "Hira_Breakpoint2.jpg",width =1200, height =1000)
...
write.table (Data_M, file ="C:/Users/FAN/Desktop/Final/Tables/Data_Hira_phylum2.csv", sep =",")

#Table 1
write.table (Coef_M, file ="C:/Users/FAN/Desktop/Final/Tables/Coef_Hira_phylum2.csv", sep =",")
...
#Distribution of cell volumes in Hira's data(Figure 2a)
png(filename = "V_P.jpg",width =1200, height =1000)
...
#Plot based on phylum (Figure 4a)
png(filename = "Hira_phylum2.jpg",width =1000, height =400) 
...
summary(fit)
Group<-tidy(fit)
write.table (Group, file ="C:/Users/FAN/Desktop/Final/Tables/Group_Hira.csv", sep =",")
